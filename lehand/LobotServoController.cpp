...
void LobotServoController::sendDataToPC(uint8_t* data, uint8_t length)
{
    uint8_t buf[length + 4];
    buf[0] = FRAME_HEADER;
    buf[1] = FRAME_HEADER;
    buf[2] = length;
    buf[3] = CMD_SEND_TO_PC;
    
    for (uint8_t i = 0; i < length; i++) {
        buf[i + 4] = data[i];
    }
    
    SerialX->write(buf, length + 4);
}

void LobotServoController::processPCCommand(uint8_t command, uint8_t* data, uint8_t length)
{
    switch(command) {
        case 0x41: // 获取设备状态
        {
            // 这里需要访问外部变量，实际使用时需要修改
            // 暂时返回默认数据
            uint8_t statusData[6] = {0, 0, 0, 0, 0, 0};
            sendDataToPC(statusData, 6);
            break;
        }
        case 0x42: // 手动设置模式
        {
	...
	while(SerialX->available() > 0)
	{
		...
		if(isGetFrameHeader)
		{
			...
			if(dataCount == 2)
			{
				...
				if(dataLength < 2 || dataLength > 10) // 增加最大长度
				{
					dataLength = 2;
					isGetFrameHeader = false;
				}
			...
			if(dataCount == dataLength + 2)
			{
				...
				switch(buf[3])
				{
					...
					case CMD_PC_COMMAND:
					{
						if(dataLength > 1) {
							uint8_t cmd = buf[4];
							uint8_t* cmdData = &buf[5];
							uint8_t dataLen = dataLength - 1;
							processPCCommand(cmd, cmdData, dataLen);
						}
						break;
					}
					default:
